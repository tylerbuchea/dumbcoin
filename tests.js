...
require("./lib/utils/consoleWrapper.js")("integrationTest", logLevel);

describe("Integration Test", () => {
  const name1 = "integrationTest1";
  const name2 = "integrationTest2";

  const createDumbcoin = (name, host, port, peers, removeData = true) => {
  ...
  step("start server 1", () => {
    return createDumbcoin(name1, "localhost", 3001, []).then((httpServer) => {
  ...
  step("start server 2", () => {
    return createDumbcoin(name2, "localhost", 3002, [
  ...
  describe("Complementary tests", () => {
    ...
    step("restart server 1", () => {
      return Promise.resolve()
        ...
        .then(() => {
          return createDumbcoin(name1, "localhost", 3001, [], false).then(
            (httpServer) => {
              context.httpServer1 = httpServer;
            }
          );
