...
const Db = require("./Db");
const { MINING_REWARD } = require("./utils/constants");
const powGetDifficulty = require("./utils/powGetDifficulty");

class Blockchain {
  constructor(dbName) {
    this.blocksDb = new Db("data/" + dbName + "/blocks.json", new Blocks());
    this.transactionsDb = new Db(
      "data/" + dbName + "/transactions.json",
      new Transactions()
    );
  ...
  getDifficulty(index) {
    ...
    return powGetDifficulty(this.blocks, index);
  }
  ...
  checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
    ...
    let sumOfOutputsAmount = R.sum(
      R.flatten(
        R.map(
          R.compose(R.map(R.prop("amount")), R.prop("outputs"), R.prop("data")),
          newBlock.transactions
        )
      )
    );
    ...
    let listOfTransactionIndexInputs = R.flatten(
      R.map(
        R.compose(
          R.map(R.compose(R.join("|"), R.props(["transaction", "index"]))),
          R.prop("inputs"),
          R.prop("data")
        ),
        newBlock.transactions
    ...
    if (transactionsByType.fee && transactionsByType.fee > 1) {
      console.error(
        `Invalid fee transaction count: expected '1' got '${transactionsByType.fee}'`
      );
      throw new Error(
        `Invalid fee transaction count: expected '1' got '${transactionsByType.fee}'`
      );
    ...
    if (transactionsByType.reward && transactionsByType.reward > 1) {
      console.error(
        `Invalid reward transaction count: expected '1' got '${transactionsByType.reward}'`
      );
      throw new Error(
        `Invalid reward transaction count: expected '1' got '${transactionsByType.reward}'`
      );
