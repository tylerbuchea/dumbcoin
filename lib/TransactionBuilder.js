...
class TransactionBuilder {
  ...
  build() {
    ...
    if (this.listOfUTXO == null)
      throw new Error(
        "It's necessary to inform a list of unspent output transactions."
      );
    if (this.outputAddress == null)
      throw new Error("It's necessary to inform the destination address.");
    if (this.totalAmount == null)
      throw new Error("It's necessary to inform the transaction value.");
    ...
    let inputs = R.map((utxo) => {
      ...
      utxo.signature = signHash(
        generateKeyPairFromSecret(self.secretKey),
        txiHash
      );
      return utxo;
    }, this.listOfUTXO);
